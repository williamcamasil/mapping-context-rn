...
describe('store', () => {
  ...
  it('should get empty data if iOS key settings SETTINGS_STORE_KEY_PREFIX has no value ', async () => {
    await Store.setItemAsync(storeKey, fakeOtpData);

    let result = await Store.getItemAsync(storeKey);
    expect(result).toEqual(fakeOtpData);

    Settings.set({
      SETTINGS_STORE_KEY_PREFIX: null,
    });

    result = await Store.getItemAsync(storeKey);
